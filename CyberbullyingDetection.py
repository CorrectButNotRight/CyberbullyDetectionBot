...
class CyberbullyingDetectionEngine:
    ...
    def load_model(self, model_name):
        ...
        """
        self.model = pickle.load(open('./models/' + model_name + 'ml_model.pkl', 'rb'))
        self.vectorizer = pickle.load(open('./models' + model_name + '_vectorizer.pkl', 'wb'))
        self.metrics = pickle.load(open('./models' + model_name + '_metrics.pkl', 'wb'))
    ...
    def train_using_tfidf(self):
        """ Trains model using ft-idf weighted words counts as features
        """
        corpus = self._simplify(self.corpus)
    ...
    def save_model(self, model_name):
        ...
        """
        pickle.dump(self.model, open('./models/' + model_name + '_ml_module.pkl', 'wb'))
...
if __name__ == '__main__':
    reddit = praw.Reddit(
        client_id = "-p43e2mGpB7BCA",
        client_secret = "qfRj4Y7zXCbIQgNw14rueKrzr5g",
        user_agent = 'reddit_script by /u/CorrectButNotRight'
        )

    new_comments = reddit.random_subreddit('TwoXChromosome').comments(limit=1000)
